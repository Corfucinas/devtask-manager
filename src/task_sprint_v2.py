...
class SprintV2:
    ...
    added_points: float = 0.0        # scope added after start
    removed_points: float = 0.0      # scope removed after start
    ...
    def scope_creep(self) -> float:
        """Net points added after sprint start."""
        return self.added_points - self.removed_points

    @property
    def load_ratio(self) -> float:
        """Committed points relative to capacity."""
        if self.capacity <= 0:
            return 0.0
...
class SprintManager:
    ...
    def add_scope(self, sprint_id: int, points: float) -> bool:
        """Record scope added mid-sprint (scope creep)."""
        s = self._sprints.get(sprint_id)
        if not s:
            return False
